...
void print_send_msg(data_msg *message) {
	printf(
			...
			message->token.locate.beg_position.c,
			message->token.locate.end_position.l,
			message->token.locate.end_position.c, message->token.color.RValue,
			message->token.color.GValue, message->token.color.BValue);
}
...
//			printf(
//					"THREAD AFFICHAGE réception trame : pos_beg: %d%d, pos_end: %d%d, couleur: %d %d %d, type : %d\n",
...
//					command.token.color.BValue, command.type);
			...
			if (command.type == WIN) {
				...
				printf(
						"DISPLAY VICTORY command type : %d, victory type : %d,"
								" pos first_token CL : %d%d, pos last_token CL : %d%d\n",
						victory_blink_infos.type,
						victory_blink_infos.victory.type,
						victory_blink_infos.victory.first_token.c,
						victory_blink_infos.victory.first_token.l,
						victory_blink_infos.victory.last_token.c,
						victory_blink_infos.victory.last_token.l);
			}
		}
	}
	return NULL;
