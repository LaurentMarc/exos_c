...
osThreadId_t leds_controlHandle;
const osThreadAttr_t leds_control_attributes = {
		.name = "Leds control",
...
int main(void)
{
  ...
  msgtosendHandle = osMessageQueueNew (1, SIZE_OF_COMMAND_PLAYER, &msgtosend_attributes);

  /* creation of msgtoreceive */
  msgtoreceiveHandle = osMessageQueueNew (16, MAX_BUFFER_SIZE, &msgtoreceive_attributes);
  ...
  /* USER CODE BEGIN RTOS_THREADS */
	...
	leds_controlHandle = osThreadNew(leds_control, NULL, &leds_control_attributes);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){ // INTERRUPTION !
	...
	else if(GPIO_Pin == Ext_Btn2_Pin){ // MILIEU GAUCHE
		btn_down = TRUE;
	}
	else if(GPIO_Pin == Ext_Btn3_Pin){ // MILIEU DROITE
		btn_down = TRUE;
	}
	else if (GPIO_Pin == Ext_Btn4_Pin){ // DROITE
		btn_right = TRUE;
	}
}
/* USER CODE END 4 */
