...
static void GreenLedBlinkTask(void *argument);

osThreadId_t GreenLedBlinkTaskHandle;
const osThreadAttr_t GreenLedBlinkTask_attributes = { .name =
		"GreenLedBlinkTask", .priority = (osPriority_t) osPriorityNormal1,
		.stack_size = 128 * 4 };

static void BlueLedBlinkTask(void *argument);

osThreadId_t BlueLedBlinkTaskHandle;
const osThreadAttr_t BlueLedBlinkTask_attributes = { .name = "BlueLedBlinkTask",
		.priority = (osPriority_t) osPriorityNormal2, .stack_size = 128 * 4 };

static void msgReaderTask(void *argument);

osThreadId_t msgReaderTaskHandle;
const osThreadAttr_t msgReaderTask_attributes = { .name = "msgReaderTask",
		.priority = (osPriority_t) osPriorityNormal3, .stack_size = 128 * 4 };
...
osMessageQueueId_t queueTaskHandle;
const osMessageQueueAttr_t queueTask_attributes = { .name = "queueTask" };
...
osSemaphoreId_t localButtonReleasedDone;
const osSemaphoreAttr_t localButtonReleasedDone_attributes = { .name =
		"localButtonReleasedDone" };
...
static void GreenLedBlinkTask(void *argument) {
	while (1) {
		...
		osSemaphoreRelease(localButtonReleasedDone);
		LedGreenOn();
		osDelay(400);
		LedGreenOff();
	}
...
static void BlueLedBlinkTask(void *argument) {
	char msg[20] = "i'm blue";
	while (1) {
		osSemaphoreAcquire(localButtonReleasedDone, osWaitForever);
		LedBlueOn();
		osMessageQueuePut(queueTaskHandle, &msg, 1, osWaitForever);
		osDelay(400);
		LedBlueOff();
	}
}
...
static void msgReaderTask(void *argument) {
	char msg[20] = { 0 };
	while (1) {
		osMessageQueueGet(queueTaskHandle, msg, NULL, osWaitForever);
		sendToSerial(msg, 20);
	}
}
int main(void) {
	...
	localButtonReleasedDone = osSemaphoreNew(1, 0,
			&localButtonReleasedDone_attributes);
	queueTaskHandle = osMessageQueueNew(16, sizeof(char[20]),
			&queueTask_attributes);
	...
	/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
	/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
	/* C'est entre ces 2 double lignes de % que vous pouvez démarrer vos   */
	/* tâches et mettre votre code perso                                     */

	defaultTaskHandle = osThreadNew(DefaultTask, NULL, &defaultTask_attributes);
	GreenLedBlinkTaskHandle = osThreadNew(GreenLedBlinkTask, NULL,
			&GreenLedBlinkTask_attributes);
	BlueLedBlinkTaskHandle = osThreadNew(BlueLedBlinkTask, NULL,
			&BlueLedBlinkTask_attributes);
	msgReaderTaskHandle = osThreadNew(msgReaderTask, NULL,
			&msgReaderTask_attributes);
